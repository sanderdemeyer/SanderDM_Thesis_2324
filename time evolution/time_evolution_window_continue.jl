...
for n = point_of_continuance+1:number_of_timesteps
    global t
    global WindowE
    global Ψt
    t = dt*n
    ...
    if (n % frequency_of_saving == 0)
        ...
        occ_numbers[div(n,frequency_of_saving),:] = get_occupation_number_matrices(Ψt.window, N, am_tilde_0, σ, x₀, S⁺, S⁻, S_z_symm)
        occ_numbers_20[div(n,frequency_of_saving),:] = get_occupation_number_matrices(Ψt.window, N, am_tilde_0, σ, 20, S⁺, S⁻, S_z_symm)
        Es[div(n,frequency_of_saving),:] = expectation_value(Ψt.window, Ht_mid(t))
    end

    if (n % 100 == 0)
        @save "continue_window_time_evolution_v_sweep_N_$(N)_mass_$(am_tilde_0)_delta_g_$(Delta_g)_ramping_$(RAMPING_TIME)_dt_$(dt)_nrsteps_$(number_of_timesteps)_vmax_$(v_max)_kappa_$(κ)_trunc_$(truncation)_savefrequency_$(frequency_of_saving)" MPSs Es occ_numbers occ_numbers_20
    end
