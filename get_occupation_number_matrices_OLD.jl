...
function get_occupation_number_matrices_right_moving(mps, L, m, σ, x₀; datapoints = div(L,2)-1, V = "old")
    N = div(L,2)-1
    @load "operators_for_occupation_number" S⁺ S⁻ S_z_symm
    corr = zeros(ComplexF64, 2*N, 2*N)
    for i = 2:2*N+1
    ...
    if V == "old"
        ...
        X = [(2*pi)/N*i for i = 0:N-1]
        X_finer = [(2*pi)/datapoints*i for i = 0:datapoints-1]
        (V₊,V₋) = V_matrix_new(X, m)
        V₊ = adjoint(V₊)
        V₋ = adjoint(V₋)
    end
    # occ_matrix = V₊*corr*adjoint(V₊) # vroeger
...
function get_occupation_number_matrices_left_moving(mps, L, m, σ, x₀; datapoints = div(L,2)-1, V = "old")
    N = div(L,2)-1
    @load "operators_for_occupation_number" S⁺ S⁻ S_z_symm
    corr = zeros(ComplexF64, 2*N, 2*N)
    for i = 2:2*N+1
