...
skip_frequency = 4
reduced_times = [dt*frequency_of_saving*skip_frequency*i for i = 0:100]#div(number_of_savepoints,skip_frequency)]
for n_t = [skip_frequency*i for i = 0:100]#div(number_of_savepoints,skip_frequency)] #1:number_of_savepoints
...
function convert_to_occ(occ_matrices_L, occ_matrices_R, X, σ, x₀)
...
σ = 0.25
x₀ = 0.5*(L-100)
# occ_numbers = convert_to_occ(occ_matrices_L, occ_matrices_R, X, σ, x₀)
...
scatter!((1-2*switch)*E_finers, occ_numbers[end], label = "t = $(125)")
scatter!((1-2*switch)*E_finers, occ_numbers[94], label = "t = $(75)")
...
plot!(E_finers, energies_convoluted, label = "kappa")
...
plt = scatter(E_finers, log.(1 ./occ_numbers[44] .- 1), label = "t = $(1.5*44)")
display(plt)
...
for i = 1:length(occ_numbers)
    push!(occ_t, occ_numbers[i][div(N,2)+10])#/maximum(occ_numbers[i]))
end
...
plt = scatter(reduced_times, entropies)
xlabel!("t")
...
@save "postprocessing_"*name Es_full entropies occ_matrices_L occ_matrices_R number_of_savepoints skip_frequency X X_finers N κ v_max times
