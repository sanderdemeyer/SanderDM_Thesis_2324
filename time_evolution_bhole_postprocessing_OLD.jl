...
for n_t = [skip_frequency*i for i = 1:div(number_of_savepoints,skip_frequency)] #1:number_of_savepoints
...
σ = 0.20 # 0.25 was used for the first figures of FD
if (L == 180)
    x₀ = 31 # used to be 31
elseif (L == 140)
    x₀ = 24
elseif (L == 100)
    ...
    error("There's probably been a mistake")
end
# x₀ = 31 # if L = 180
# x₀ = 24 # if L = 140
...
if (Delta_g == 0.0)
    factor = 1.0
elseif Delta_g == -0.15
    @assert (am_tilde_0 ≈ 0.06)
    factor = 0.995073/0.839257
else
    error("Deltag is wrong?")
end
...
scatter!((1-2*switch)*E_finers, occ_numbers[50], label = "t = $(reduced_times[50])")
scatter!((1-2*switch)*E_finers, occ_numbers[80], label = "t = $(reduced_times[80])")
...
xlabel!("Time")
ylabel!("Occ numb for k = $(-round(X_finers[1][div(N,2)+momentum],digits=3)), |E| = $(round(E_finers[div(N,2)+momentum],digits=3))")
hline!([energies_convoluted[div(N,2)+momentum]], label = "expected")
display(plt)
...
# @load name_save Es_full entropies occ_matrices_L occ_matrices_R number_of_savepoints skip_frequency X N κ v_max times
