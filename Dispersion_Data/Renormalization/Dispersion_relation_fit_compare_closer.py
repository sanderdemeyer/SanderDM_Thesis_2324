...
rounding = 3
schmidt_cut = 3.5
excluded_0 = 0 # do you want to ignore m = 0.0?

delta_g_number = 3
mass_number = 8
v_number = 1
schmidt_cut_number = 1
...
for (ind, closer) in enumerate([1 + i for i in range(max_index)]):
    ...
    (mass_renorm, v_renorm, c_renorm, mass_sigma_renorm, v_sigma_renorm, c_sigma_renorm, mu_value) = drff.Dispersion_relation_fit(closer, way_of_fitting=way_of_fitting, plot = False)
...
# with open("masses_renormalization.csv", 'w', newline='') as csvfile:
#     csvwriter = csv.writer(csvfile)
#     csvwriter.writerows(mass_renorms)
# with open("cs_renormalization.csv", 'w', newline='') as csvfile:
#     csvwriter = csv.writer(csvfile)
#     csvwriter.writerows(c_renorms)

function_to_fit = fit_4th_order

show = ["c"] # plotting renormalized value of v when v = 0 makes no sense
error_bars = True
extrapolation_in_factor = True
...
def fitted_values(data, fit_function_used='parabola'):
    ...
    return (fitted_values_fit, popt[-1], pcov[-1])
    ...
    for m_index in range(1,7):
        for (i,delta_g) in enumerate([-0.15*i for i in range(1, delta_g_range)]):
            if error_bars:
                ...
                if extrapolation_in_factor:
                    (fitting, popt, pcov) = fitted_values(c_renorms[:,0,i,m_index-excluded_0])
                    plt.plot(parabola_factor_values, fitting, label = 'fitted function')
                    print(rf"For m = {round(m_index*0.1,2)}, delta_g = {delta_g}, optimal value is {popt} $\pm$ {pcov}")
            else:
...
print(v_renorms[0][0])
print(mass_renorms[0][0])
print(c_renorms[0][0])

print(v_renorms[0][0]/(-0.5))
mass_renorms_rel = copy.copy(mass_renorms[0][0])
for i in range(2):
    for j in range(8):
        mass_renorms_rel[i,j] /= (j+3)*0.01

print(mass_renorms_rel)

print("uncertainties")
print(v_sigma_renorms[0][0])
print(mass_sigma_renorms[0][0])
print(c_sigma_renorms[0][0])

# print("intentionally breaking")
# assert(0 == 1)
for m_index in range(0, 8):
    plt.scatter([-0.15*i for i in range(1, delta_g_range)], mu_values[0,0,:,m_index-excluded_0], label = f'mass = {round((m_index+3)*0.01,2)}')
...
for m_index in range(0, 8):
    plt.errorbar([-0.15*i for i in range(1, delta_g_range)], v_renorms[0,0,:,m_index-excluded_0]/(v), v_sigma_renorms[0,0,:,m_index-excluded_0]/abs(v), label = f'mass = {round((m_index+3)*0.01,2)}', fmt="o")
    #plt.errorbar([-0.15*i for i in range(1, 5)], v_renorms[0,0,:,m_index-excluded_0]/(-0.15), v_sigma_renorms[0,0,:,m_index-excluded_0]/(0.15), label = f'mass = {round(m_index*0.1,2)}. schmidt = {schmidt_cut}', fmt="o")
...
for m_index in range(0, 8):
    plt.errorbar([-0.15*i for i in range(1, delta_g_range)], mass_renorms[0,0,:,m_index-excluded_0]/((m_index+3)*0.01), mass_sigma_renorms[0,0,:,m_index-excluded_0]/((m_index+3)*0.01), label = f'mass = {round((m_index+3)*0.1,2)}', fmt="o")
...
for m_index in range(0, 8):
    plt.errorbar([-0.15*i for i in range(1, delta_g_range)], c_renorms[0,0,:,m_index-excluded_0], c_sigma_renorms[0,0,:,m_index-excluded_0], label = f'mass = {round((m_index+3)*0.01,2)}', fmt="o")
