...
def fit_function(delta_g, v, mass, data_folder, closer, plot = False):
    ...
    #file = f"Dispersion_Data/Renormalization/{data_folder}/Dispersion_m_{mass} _delta_g_{delta_g} _v_{v}"
    file = f"Dispersion_Data/Renormalization/{data_folder}/Dispersion_closer_m_{mass} _delta_g_{delta_g} _v_{v}"
    # file = f"Dispersion_Data/Renormalization/{data_folder}/Dispersion_pi_over_72_m_{mass} _delta_g_{delta_g} _v_{v}"
...
def fit_function_factor_efficient(delta_g, v, mass, data_folder, factor, plot = False):
    ...
    if False:
        ...
        #file = f"Dispersion_Data/Renormalization/{data_folder}/Dispersion_m_{mass} _delta_g_{delta_g} _v_{v}"
        #file = f"Dispersion_Data/Renormalization/{data_folder}/Dispersion_closer_m_{mass} _delta_g_{delta_g} _v_{v}"
        file = f"Dispersion_Data/Renormalization/{data_folder}/Dispersion_pi_over_72_v_0_m_{mass}_delta_g_{delta_g}_v_{v}_trunc_2.5"
        anti_file = f"Dispersion_Data/Renormalization/{anti_data_folder}/Dispersion_pi_over_72_v_0_U1_-1_m_{mass}_delta_g_{delta_g}_v_{v}_trunc_2.5"
        ...
        data_folder = "Data_small_m_pi_over_288"
        file = f"Dispersion_Data/Renormalization/{data_folder}/Dispersion_pi_over_288_v_{v}_m_{mass}_delta_g_{delta_g}_v_{v}_trunc_2.0_all_sectors"
        f = h5py.File(file, "r")
        energies = f["energies"][:]
    ...
    popt, pcov = curve_fit(function_fit, k_points, energy_points, (mass, v), maxfev=100000)
...
def Dispersion_relation_fit(closer, way_of_fitting = "factor", plot = False):
    ...
    for (schmidt_number, schmidt_cut) in enumerate([2.0]):
        ...
        for (delta_g_index,delta_g) in enumerate([-0.15*i for i in range(1, delta_g_number)]): # 4 should be 5 sometimes
            for (mass_index,mass) in enumerate([0.01*(i+3) for i in range(0, mass_number)]):
                v = 0.5
